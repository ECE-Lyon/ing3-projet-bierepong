...
public class FenetreLoginGrid extends JFrame {
    public FenetreLoginGrid() {
        ...
        ButSubmitEmploye.addActionListener(e -> {
            ...
            try (Connection con = DriverManager.getConnection("jdbc:h2:./default"); PreparedStatement statement = con.prepareStatement("select MAIL,PASSWORD from employee")) { //ATT EMPLOYES!!!
                try (ResultSet resultSet = statement.executeQuery()) {
                    if(resultSet.next()) {
                        String mail = resultSet.getString("mail");
                        String password = resultSet.getString("password");
